/* ==========================================================================
   ...
   groszen Terz, und das ist richtig so.
...
function renderLeiter(root) {
  ...
  host.innerHTML = [...reihe].reverse().map(p => {
    ...
    return `<div class="ob-stufe${aktiv ? " an" : ""}${istZiel || gezielt ? " ziel" : ""}">
      <span class="ob-k">${p.k}</span>
      <span class="ob-ton">${escapeHtml(teiltonName(p))}</span>
      <span class="ob-cent">${p.cents > 0 ? "+" : ""}${p.cents}</span>
      <span class="ob-bar"><i style="width:${aktiv ? Math.max(6, 100 - Math.min(100, Math.abs(erkannt.abweichungCents) * 2)) : 0}%"></i></span>
    </div>`;
  }).join("");
}
